...
- `boot` -> `core` lifecycle role alias
- `root` -> `core` control ingress role alias
- `kernel` -> `core` sovereign role alias
- `engine` -> `exec` role alias
- `mind` -> `brain` role alias

## Deprecated terms / migration aliases

Deprecated-as-primary:
- `kernel boundary` (replace with `sovereign core boundary`)
- `engine boundary` (replace with `execution boundary`)
- `mind boundary` (replace with `cognitive boundary`)
